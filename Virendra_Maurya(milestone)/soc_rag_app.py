...
# Notes, tips, and explanation

# The script is defensive: it runs even if FAISS or rank_bm25 are not installed (it falls back to numpy/TF-IDF).

# The hybrid retriever computes vector similarity (embedding) and text-similarity (BM25 or TF-IDF) and combines with weights (vector 0.7, text 0.3).

# Per-analyst InMemoryChatMessageHistory is used via RunnableWithMessageHistory to inject past history automatically. Entity memory (_sessions_entity_memory) is separately maintained and injected into the prompt as a JSON string.

# The threat_enrich tool is implemented and bind_tools is shown — the code also attempts to detect tool calls in the LLM output and resolve them (defensively).

# compute_threat_score is a simple rule-based scoring function combining severity, presence of IPs/MITRE tags and suspicious keywords — tweak as needed.

# Final output includes a structured JSON (structured) with retrieved refs, entities, score, and full assistant text.

# For production: replace InMemoryChatMessageHistory with a persistent memory and use a proper vector DB (Faiss/Chroma/Pinecone), plus more robust entity extraction (NER) and threat enrichment integrati